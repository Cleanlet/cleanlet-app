...
import '../components/cleanup_steps.dart';
...
class VolunteerButton extends ConsumerWidget {
  const VolunteerButton(this.jobId, this.inletId, {
    Key? key,
  }) : super(key: key);
  final String jobId;
    ...
    return job.when(
        data: (job) {
          return Container(
            ...
            }, icon: const Icon(Icons.check_box), label: const Text('Volunteer to clean inlet'), style: OutlinedButton.styleFrom(minimumSize: const Size.fromHeight(
                40)),),
          );
        }, error: (error, stack) => const Text('Error'), loading: () => const Text('Loading...')
    );
...
class StartButton extends ConsumerWidget {
  ...
  build(BuildContext context, WidgetRef ref) {
    return Container(
      ...
      }, icon: const Icon(Icons.check_box), label: const Text('Start Cleaning'), style: OutlinedButton.styleFrom(minimumSize: const Size.fromHeight(
              40)),),
    );
  }
  ...
  build(BuildContext context, WidgetRef ref) {
    ...
    if (inlet.status == 'cleaningScheduled') {
      ...
      return Container(
        ...
        }, icon: const Icon(Icons.notification_add), label: const Text('Start Cleaning'), style: OutlinedButton.styleFrom(minimumSize: const Size.fromHeight(
            40)),),
      );
